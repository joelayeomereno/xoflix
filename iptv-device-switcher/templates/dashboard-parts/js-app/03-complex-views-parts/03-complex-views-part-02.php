<?php if ( ! defined( 'ABSPATH' ) ) exit; ?>
        ...
        try {
            ...
            if (result.success) {
                 ...
                 if (msg.includes('session')) {
                     ...
                     // PREM FIX: Activate High-End Feedback Modal
                     setFeedback({
                         show: true,
            ...
            setFeedback({
                ...
                message: "A communication error occurred with our server clusters. Please verify your connection and attempt the request again.",
                type: "error"
            });
            setIsSubmitting(false);
        }
    ...
    // --- MODERN PREMIUM FEEDBACK MODAL ---
    const FeedbackModal = ({ data, onClose }) => {
        if (!data.show) return null;
        return (
            <div className="fixed inset-0 z-[99999] flex items-center justify-center p-6 animate-fade-in">
                <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-xl transition-all" onClick={onClose}></div>
                <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.4)] relative z-10 animate-slide-up overflow-hidden border border-white/20">
                    {/* Top Decorative bar */}
                    <div className={`h-1.5 w-full ${data.type === 'error' ? 'bg-rose-500' : 'bg-indigo-500'}`}></div>
                    
                    <div className="p-10 text-center">
                        <div className={`w-20 h-20 rounded-[1.5rem] flex items-center justify-center mx-auto mb-8 transition-transform transform hover:scale-110 duration-500 ${data.type === 'error' ? 'bg-rose-50 text-rose-500' : 'bg-indigo-50 text-indigo-500'}`}>
                            {data.type === 'error' ? (
                                <AlertTriangle size={40} strokeWidth={2.5} />
                            ) : (
                                <div className="relative">
                                    <div className="absolute inset-0 animate-ping bg-indigo-400/20 rounded-full"></div>
                                    <Clock size={40} strokeWidth={2.5} className="relative z-10" />
                                </div>
                            )}
                        </div>
                        
                        <h3 className="text-2xl font-black text-slate-900 tracking-tight mb-4">{data.title}</h3>
                        <p className="text-[15px] text-slate-500 font-medium leading-relaxed mb-10 px-2">
                            {data.message}
                        </p>
                        
                        <button 
                            onClick={onClose}
                            className="w-full py-5 bg-slate-900 hover:bg-black text-white font-black rounded-2xl shadow-xl shadow-slate-200 transition-all active:scale-95 uppercase tracking-widest text-xs"
                        >
                            Return to Dashboard
                        </button>
                    </div>
    ...
    const [feedback, setFeedback] = useState({ show: false, title: "", message: "", type: "info" });
    ...
    return (
        <div className="animate-slide-up max-w-4xl mx-auto relative">
            ...
            {/* INJECT FEEDBACK COMPONENT */}
            <FeedbackModal data={feedback} onClose={() => setFeedback({ ...feedback, show: false })} />
