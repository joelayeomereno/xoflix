...
/**
 * Trait: TV_Subscription_Manager_Admin_Classes_TV_Admin_Users_Trait_Part_02
 * Path: /tv-subscription-manager/admin/classes/tv-admin-users/trait-tv-subscription-manager-admin-classes-tv-admin-users-trait-part-02.php
 */
trait TV_Subscription_Manager_Admin_Classes_TV_Admin_Users_Trait_Part_02 {
    ...
    public function handle_impersonation_start() {
        if (isset($_GET['action']) && $_GET['action'] === 'start_impersonation' && isset($_GET['user_id'])) {
            ...
            // [FIX] Explicitly cast to INT to match wp_nonce_url generation logic
            $target_user_id = intval($_GET['user_id']);
            
            // Verify Nonce with sanitized ID
            check_admin_referer('start_impersonation_' . $target_user_id);
            ...
            // Log event (if logging service exists)
    ...
    public function handle_impersonation_close() {
        if (isset($_GET['action']) && $_GET['action'] === 'close_session' && isset($_GET['target_id'])) {
            ...
            // [FIX] Consistent int casting for close action as well
            $target_id = intval($_GET['target_id']);
            check_admin_referer('close_session_' . $target_id);
