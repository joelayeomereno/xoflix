...
trait TV_Admin_Users_Trait_Render {
    ...
    private function render_subscriber_detail_view() {
        ...
        if (file_exists(TV_MANAGER_PATH . 'admin/views/view-subscriber-detail.php')) {
            ...
            // Fallback to the new manage view
    ...
    private function render_users_hub_view() {
        ...
        // Latest subscription per user (by max id), plus plan join for display/sorting.
        $sql_base = "
    ...
    /**
     * Manage a single user (profile + subscriptions + transactions).
     * This view is routed via: ?page=tv-subs-manager&tab=users&action=manage&user_id=123
     */
    private function render_user_manage_view() {
        ...
        // Strict MANAGE segmentation (server-side scoping)
        $manage_section = isset($_GET['manage_section']) ? sanitize_key((string)$_GET['manage_section']) : 'profile';
        if (!in_array($manage_section, array('profile', 'subscription', 'transactions'), true)) {
            $manage_section = 'profile';
        }

        // Defaults to avoid notices in the view.
        $plans = array();
        $all_subs = array();
        $sub = null;
        $payments = array();
        ...
        if ($manage_section === 'subscription') {
            ...
            // All subscriptions + plan name for this user.
            $all_subs = $this->wpdb->get_results(
                $this->wpdb->prepare(
            ...
            // Allow selecting which subscription is being edited.
            $edit_sub_id = isset($_GET['sub_id']) ? (int)$_GET['sub_id'] : 0;
            if (!empty($all_subs)) {
                if ($edit_sub_id > 0) {
                    foreach ($all_subs as $candidate) {
                        if ((int)$candidate->id === $edit_sub_id) { $sub = $candidate; break; }
                    }
                }
                if (!$sub) {
                    $sub = $all_subs[0];
            ...
            $payments = $this->wpdb->get_results(
                $this->wpdb->prepare(
                    "SELECT * FROM $this->table_payments WHERE user_id = %d ORDER BY id DESC LIMIT 50",
                    $user_id
                )
            );
        }
