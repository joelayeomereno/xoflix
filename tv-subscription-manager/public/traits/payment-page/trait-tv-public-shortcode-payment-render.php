<?php
if ( ! defined( 'ABSPATH' ) ) exit;

trait TV_Public_Shortcode_Payment_Render_Trait {

    public function shortcode_payment_page($atts) {
            if (!headers_sent()) nocache_headers();
            if(!is_user_logged_in()) return '<div class="tv-error">Please log in.</div>';
    
            global $wpdb;
            $user_id = get_current_user_id();

            // -------------------------------------------------------------------
            // 1. PER-SUBSCRIPTION LOCKDOWN (not per-user)
            //
            // A user is allowed to purchase multiple plans simultaneously.
            // The lock must therefore only apply when the user is trying to
            // extend/renew a SPECIFIC subscription that already has an open
            // (actionable) payment attached to it.
            //
            // Rules:
            //   target_sub_id = 0  ? brand new subscription ? ALWAYS allowed
            //   target_sub_id > 0  ? extending an existing sub ? BLOCKED if that
            //                         sub already has a payment in an actionable state
            //
            // Actionable statuses (payment is in-flight, do not duplicate):
            //   IN_PROGRESS, AWAITING_PROOF, PENDING_ADMIN_REVIEW, APPROVED
            //
            // Terminal statuses (safe to allow new payment):
            //   COMPLETED, REJECTED, CANCELLED, FAILED
            // -------------------------------------------------------------------

            // Peek at the pending checkout session to read target_sub_id BEFORE
            // we create anything, so we can decide whether to block.
            $pending_peek = get_user_meta($user_id, '_tv_pending_checkout', true);
            $peek_sub_id  = (isset($pending_peek['target_sub_id']) && intval($pending_peek['target_sub_id']) > 0)
                                ? intval($pending_peek['target_sub_id'])
                                : 0;

            // Also check POST for direct form submissions (first-hit entry point)
            if ($peek_sub_id === 0 && isset($_POST['target_subscription_id'])) {
                $peek_sub_id = intval($_POST['target_subscription_id']);
            }

            if ($peek_sub_id > 0) {
                // Check if this specific subscription already has an open actionable payment
                $actionable_statuses = ['IN_PROGRESS', 'AWAITING_PROOF', 'pending', 'PENDING_ADMIN_REVIEW', 'APPROVED'];
                $placeholders        = implode(',', array_fill(0, count($actionable_statuses), '%s'));
                $open_payment        = $wpdb->get_row($wpdb->prepare(
                    "SELECT id, status FROM {$wpdb->prefix}tv_payments
                     WHERE subscription_id = %d
                       AND user_id         = %d
                       AND status IN ($placeholders)
                     ORDER BY id DESC
                     LIMIT 1",
                    array_merge([$peek_sub_id, $user_id], $actionable_statuses)
                ));

                if ($open_payment) {
                    // This subscription already has an open payment in progress —
                    // redirect to the pending page so the user can track it.
                    update_user_meta($user_id, '_tv_active_pay_id', (int)$open_payment->id);
                    return '<script>window.location.href="' . add_query_arg(['tv_flow' => 'payment_pending'], home_url('/')) . '";</script>';
                }
            }

            // Clean up stale _tv_active_pay_id meta if the payment is terminal.
            // This never blocks a new purchase — it just prevents a dead pointer
            // from lingering in user meta indefinitely.
            $active_pay_id = (int)get_user_meta($user_id, '_tv_active_pay_id', true);
            if ($active_pay_id) {
                $active_status = $wpdb->get_var($wpdb->prepare(
                    "SELECT status FROM {$wpdb->prefix}tv_payments WHERE id = %d AND user_id = %d",
                    $active_pay_id, $user_id
                ));
                if (!$active_status || in_array(strtoupper((string)$active_status), ['COMPLETED', 'REJECTED', 'CANCELLED', 'FAILED'])) {
                    delete_user_meta($user_id, '_tv_active_pay_id');
                }
            }

            // -------------------------------------------------------------------
            // 2. NEW SESSION SETUP
            // -------------------------------------------------------------------
            $pending = get_user_meta($user_id, '_tv_pending_checkout', true);
            
            if ((empty($pending) || !is_array($pending)) && isset($_POST['plan_id'])) {
                $this->handle_subscription_request();
                $pending = get_user_meta($user_id, '_tv_pending_checkout', true);
            }
    
            if (empty($pending) || !is_array($pending)) {
                return '<script>window.location.href="' . add_query_arg('tv_flow', 'select_method', home_url('/')) . '";</script>';
            }
    
            // -------------------------------------------------------------------
            // 3. RETRIEVE DATA
            // -------------------------------------------------------------------
            $plan_id       = intval($pending['plan_id']);
            $method_id     = intval($pending['payment_method_id']);
            $connections   = isset($pending['connections']) ? intval($pending['connections']) : 1;
            $custom_months = isset($pending['custom_months']) ? intval($pending['custom_months']) : 0;
            $target_sub_id = isset($pending['target_sub_id']) ? intval($pending['target_sub_id']) : 0;
    
            $plan   = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}tv_plans WHERE id = %d", $plan_id));
            $method = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}tv_payment_methods WHERE id = %d", $method_id));
            
            if (!$plan || !$method) {
                delete_user_meta($user_id, '_tv_pending_checkout');
                return '<script>window.location.href="' . home_url('/dashboard') . '";</script>';
            }
    
            // -------------------------------------------------------------------
            // 4. CALCULATE FINALS
            // -------------------------------------------------------------------
            $base_price_usd   = floatval($plan->price);
            $months           = ($custom_months > 0) ? $custom_months : 1;
            $connections      = max(1, (int) $connections);
            
            $local_unit_price = $base_price_usd;
            $final_currency   = 'USD';
            $currency_data    = [];
            
            if (method_exists($this, 'get_currency_data')) {
                $currency_data = $this->get_currency_data($base_price_usd);
                if (is_array($currency_data)) {
                    $local_unit_price = $currency_data['amount_local']; 
                    $final_currency   = $currency_data['code'];
                }
            }
    
            $gross_local  = $local_unit_price * $months * $connections;
            $final_amount = $gross_local;
            
            $coupon_code = isset($pending['coupon_code']) ? sanitize_text_field($pending['coupon_code']) : '';
            if ($coupon_code) {
                $coupon = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}tv_coupons WHERE code = %s", $coupon_code));
                if ($coupon) {
                    if ($coupon->type === 'percent') {
                        $discount_val = ($final_amount * ($coupon->amount / 100));
                        $final_amount = $final_amount - $discount_val;
                    } elseif ($coupon->type === 'fixed') {
                        $coupon_val_usd = floatval($coupon->amount);
                        $discount_val   = $coupon_val_usd;
                        if (method_exists($this, 'get_currency_data')) {
                            $coupon_data  = $this->get_currency_data($coupon_val_usd);
                            $discount_val = $coupon_data['amount_local'];
                        }
                        if ($discount_val > $final_amount) $discount_val = $final_amount;
                        $final_amount = $final_amount - $discount_val;
                    }
                }
            }
    
            // -------------------------------------------------------------------
            // 5. BUILD PAYMENT RECORD
            // -------------------------------------------------------------------
            $cycle_days = $plan ? max(1, (int)$plan->duration_days) : 1;
            $total_days = max(1, (int)$months) * $cycle_days;
            
            $amount_usd = $final_amount;
            $fx_rate    = 1;
            
            if (!empty($currency_data) && isset($currency_data['rate'])) {
                $fx_rate    = (float)$currency_data['rate'];
                $amount_usd = ($fx_rate > 0) ? ($final_amount / $fx_rate) : $final_amount;
            }

            $gross_usd      = ($fx_rate > 0) ? ($gross_local / $fx_rate) : $gross_local;
            $net_local      = $final_amount;
            $net_usd        = $amount_usd;
            $discount_local = max(0, $gross_local - $net_local);
            $discount_usd   = max(0, $gross_usd - $net_usd);
            
            $amount_ngn = 0;
            if (method_exists($this, 'get_ngn_rate')) {
                $ngn_rate = $this->get_ngn_rate();
                if ($ngn_rate > 0) {
                    $amount_ngn = $net_usd * $ngn_rate;
                }
            }
            
            $temp_txn_id = 'TMP-' . strtoupper(uniqid());
    
            $created = $this->create_subscription_and_payment(
                $user_id,
                $plan_id,
                $method,
                $final_amount,
                $total_days,
                $connections,
                $coupon_code,
                $final_currency,
                $months,
                $net_usd,
                $fx_rate,
                $discount_usd,
                $discount_local,
                $gross_usd,
                $gross_local,
                $target_sub_id,
                $amount_ngn,
                $temp_txn_id
            );
            
            if ($created && isset($created['pay_id'])) {
                update_user_meta($user_id, '_tv_active_pay_id', $created['pay_id']);
                $wpdb->update(
                    "{$wpdb->prefix}tv_payments",
                    ['attempted_at' => current_time('mysql'), 'status' => 'IN_PROGRESS'],
                    ['id' => $created['pay_id']]
                );
                $lockdown_url = add_query_arg('tv_flow', 'payment_pending', home_url('/'));
                wp_redirect($lockdown_url);
                exit;
            } else {
                return '<div class="tv-error">Error creating payment session. Please try again.</div>';
            }
        }

}