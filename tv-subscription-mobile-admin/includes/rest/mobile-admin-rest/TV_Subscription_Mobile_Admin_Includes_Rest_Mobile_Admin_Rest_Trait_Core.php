...
trait TV_Subscription_Mobile_Admin_Includes_Rest_Mobile_Admin_Rest_Trait_Core {
     ...
     * Never use HTML entities here — this is a REST API, not an HTML template.
     *
     * FIX: old fallback contained garbled/broken placeholder characters.
     *      TV_Currency::symbol() is now the canonical source after class-tv-currency.php
     *      was also fixed to return Unicode instead of HTML entities.
     */
    private static function currency_symbol(string $code) : string {
        $code = strtoupper(trim($code));
        if ($code === '') return '';

        // Prefer TV_Currency when available — it now returns proper Unicode.
        if (class_exists('TV_Currency')) {
            return TV_Currency::symbol($code);
        }

        // Emergency fallback (TV_Currency should always be present when both
        // plugins are active together, but guard just in case).
        $fallback = [
            'USD' => '$',
            'EUR' => "\u{20AC}",  // €
            'GBP' => "\u{00A3}",  // £
