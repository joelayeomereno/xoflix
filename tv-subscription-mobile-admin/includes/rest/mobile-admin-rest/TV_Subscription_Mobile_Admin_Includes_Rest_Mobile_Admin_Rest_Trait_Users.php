 ...
 *       TV_Subscription_Mobile_Admin_Includes_Rest_Mobile_Admin_Rest_Trait_Users.php
 *
 * UPGRADES vs original:
 * -----------------------------------------------------------------------
 * [NEW] get_user_details now returns a complete payload matching desktop TV Manager:
 *   - profile: id, user_login, email, display_name, first_name, last_name,
 *              phone, billing_country, admin_notes, user_registered
 *   - subscriptions: full s.* (includes credential_user/pass/url/m3u) + plan_name
 *   - payments:  per-user payment history (id, date, amount, currency, currency_symbol,
 *                amount_display, method, status, transaction_id, plan_name, proofs)
 *   - ltv:       lifetime value — SUM(amount) of completed/approved payments
 *   - logs:      last 50 activity log rows for this user
 *   - impersonate_url: unchanged
 * -----------------------------------------------------------------------
 */
trait TV_Subscription_Mobile_Admin_Includes_Rest_Mobile_Admin_Rest_Trait_Users {
     ...
     * GET /users/{id}
     *
     * Returns everything the desktop TV Manager user-manage page shows:
     *   profile, subscriptions (with credentials), payments, ltv, logs, impersonate_url
     */
    public static function get_user_details( WP_REST_Request $req ) : WP_REST_Response {
        ...
        // ---- Subscriptions (full row incl. credentials) ----------------------
        ...
        // ---- Payments (full history for this user) ---------------------------
        ...
        // ---- Lifetime Value (sum of completed/approved payments) -------------
    ...
    // -----------------------
    // System Settings (next section marker)
    // -----------------------
